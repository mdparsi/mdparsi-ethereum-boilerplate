...
function DEX({ chain, customTokens = {} }) {
  ...
  const ButtonState = useMemo(() => {
    if (chainIds?.[chainId] !== chain)
      return { isActive: false, text: `تغییر به ${chain}` };

    if (!fromAmount) return { isActive: false, text: "مقدار وارد کنید" };
    if (fromAmount && currentTrade) return { isActive: true, text: "مبادله" };
    return { isActive: false, text: "توکن را انتخاب کنید" };
