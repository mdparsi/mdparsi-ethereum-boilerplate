...
🚀DEMO: https://mdparsi.github.io/mdparsi-ethereum-boilerplate
...
در این استارتر بسیاری از مؤلفه ها وجود دارد که نیازی به ارائه دهنده فعال Web3 ندارند ، آنها از API Moralis Web3 استفاده می کنند.Moralis از محبوب ترین blockchain ها و شبکه های آزمایشی آنها پشتیبانی می کند.می توانید لیستی از کلیه شبکه های موجود را در پیدا کنید [Moralis Supported Chains](https://docs.moralis.io/moralis-server/web3-sdk/intro#supported-chains)

لطفا [مستندات رسمی مرالس](https://docs.moralis.io/#user)  ... را چک کنید برای دیدن همه ویژگی های مرالس.
...
اگر در تنظیم دیگ بخار یا سوالات دیگری به کمک نیاز دارید - از نوشتن در انجمن جامعه ما دریغ نکنید و ASAP را بررسی خواهیم کرد. [لینک فروم](https://forum.moralis.io/t/ethereum-boilerplate-questions/3951/86). بهترین چیز در مورد این دیگ بخار ، جامعه فوق العاده فعال آماده کمک در هر زمان است!ما به یکدیگر کمک می کنیم.
...
✏  فایل `.env.example` را به `.env` تغییر نام دهید و شناسه `appId` و آدرس سرور `serverUrl` مرالس خود را وارد کنید ([چگونه سرور Moralis را شروع کنیم](https://docs.moralis.io/moralis-server/getting-started/create-a-moralis-server))
مثال:
...
📫 `<AddressInput />` : ورودی برای آدرس ETH.نمایش [Blockie](https://www.npmjs.com/package/react-blockies) آواتار برای کیف پول وارد شده.به اعتبارسنجی آدرس کمک می کند.پس از ورود 42 نویسه (0x... wallet length) یا دامنه رمزنگاری معتبر ورودی و تماس ها را یخ می زند `setValidatedAddress`.
...
⛓ `<Chains />` :سوئیچ شبکه فعال.از blockchain های اتریوم ، پالیگان ، BSC و Avalacnhe پشتیبانی می کند.فقط با شبکه هایی که قبلاً به کیف پول تزریق شده اضافه شده اند ، کار می کند.می توانید راهنمایی در مورد چگونگی اضافه کردن برنامه ای یک شبکه جدید [اینجا] پیدا کنید(https://docs.moralis.io/moralis-server/web3/web3#addnetwork). به راحتی قابل تنظیم است ، می توانید شبکه های دیگر را اضافه کنید

**گزینه ها**:

- غرفه ها (اختیاری): شبکه هایی برای نمایش.به طور پیش فرض اضافه شده است: پالیگان ، ETH ، BSC و بهمن
...
💵 `<CoinPrice />` : قیمت نشانه مشخص شده در تنظیمات را نشان می دهد.از Moralis Web3API استفاده می کند (به ارائه دهنده فعال Web3 احتیاج ندارد).

**گزینه ها**:

- آدرس (لازم): آدرس قرارداد توکن
- زنجیره ای (اختیاری): شبکه ای که توکن مستقر شده است.پیش فرض: ETH
- تصویر (اختیاری): مسیر محلی یا لینک به لوگو توکن
- اندازه (اختیاری): اندازه لوگو
...
- زنجیره (اختیاری): شبکه برای نمایش مانده در.اگر خودتان زنجیره ای را مشخص نکنید ، از شبکه کیف پول خود استفاده خواهید کرد
...
- زنجیره (اختیاری): شبکه برای نمایش نقل و انتقالات.اگر خودتان زنجیره ای را مشخص نکنید ، از شبکه کیف پول خود استفاده خواهید کرد
...
- زنجیره (اختیاری): شبکه. موجود: ethereum ("eth")، زنجیره هوشمند Binance ("bsc")، Polygon ("پالیگان")
- customTokens("توکن سفارشی") (اختیاری): مورد توکن سفارشی. نمونه زیر را می توانید ببینید.
...
💼 `<Wallet />` : رابط مثال برای تعامل با کیف پول خود.از اجزای موجود در استارتر استفاده می کند: `<Blockie />`, `<Address />`, `<NativeBalance />`, `<AddressInput />`. قابلیت ارسال توکن‌ها را دارد
...
📋 عملکرد مشخصی از قرارداد ABI را اجرا می کند و داده های Readonly را برمی گرداند.از مرالس Web3API استفاده می کند (به ارائه دهنده فعال Web3 احتیاج ندارد).
